...
/**
 * A subsystem to control the LEDs on the robot. The LEDs are controlled by
 * an {@link AddressableLED} object, which is used to set the color of each
 * LED in the strip. The LEDs are divided into segments, each of which can be
 * controlled independently. The segments are defined in the {@link Segments}
 * enum, which implements the {@link LEDReader} and {@link LEDWriter} interfaces.
 * 
 * The LEDs can be controlled by setting the color of individual pixels, or by
 * applying a pattern to a segment or group of segments. The {@link LEDs} class
 * provides methods to set the color of individual pixels, apply patterns to
 * segments, and display information on the LEDs.
 * 
 * This is a singleton class.  If more AddressableLED objects are needed, they
 * can be added alongside ther main LED object, and new segments can be defined
 * in the {@link Segments} enum.  In this case, the update() method would need
 * to be updated to include the new LED objects.
 */
public class LEDs extends SubsystemBase {

  private static LEDs instance;
  private static final AddressableLED mainLed = new AddressableLED(LedConstants.kLedPort);
  private static final AddressableLEDBuffer mainLedBuffer = new AddressableLEDBuffer(LedConstants.kLedPixelCount);
  ...
  /**
   * Enum to define the segments of the LED strip. Each segment is defined by
   * an {@link AddressableLED} object, an {@link AddressableLEDBuffer} object,
   * a start index, an end index, and a boolean indicating whether the segment
   * is reversed. The enum implements the {@link LEDReader} and {@link LEDWriter}
   * interfaces, so LEDPattern objects can be applied to the segments.
   */
  ...
  /**
   * Get the singleton instance of the LEDs subsystem. If the instance does not
   * exist, create it and start the main LED strip.
  ...
  /**
   * Creates a number of blocks of a given color, with a given size and space
   * between each block, and stacks them on the provided LEDWriters
   * .
  ...
  /**
   * Sets <i>n</i> LED blocks to the alliance color, starting from the bottom, on both
   * the left and right strips, where <i>n</i> corresponds to the auto mode selected.
   * If no auto is selected, two yellow pixels are lit at the bottom of each strip,
   * and if the alliance inputs disagree, two yellow pixels are lit at the top of
   * each strip.
   * 
  ...
  /**
   * Uses the middle segments to indicate the control mode, and the top and bottom
   * segments to indicate the gamepiece currently possessed by the robot.
   * 
   * The middle segments are lit in the coral color if the robot is in coral mode,
   * and in the algae color if the robot is in algae mode. The top and bottom segments
   * are lit in the color of the gamepiece if the robot possesses one, else black.
   * 
  ...
  /**
   * Use LEDs to indicate how to move the robot relative to its current position to reach the
   * target pose. The directions are as follows:
   *
   * <ul>
   *   <li><b>Heading</b> how to face the robot in the correct direction:
   *     <ul>
   *       <li>Use middle segments:</li>
   *       <li>Both white indicates proper heading.</li>
   *       <li>Both blue means rotate clockwise.</li>
   *       <li>Both magenta means rotate counter-clockwise.</li>
   *     </ul>
   *   </li>
   *   <li><b>X</b> Use top segments:
   *     <ul>
   *       <li>Green means move forward.</li>
   *       <li>Red means move backward.</li>
   *     </ul>
   *   </li>
   *   <li><b>Y</b> Use bottom segments:
   *     <ul>
   *       <li>Illuminate the corresponding side.</li>
   *     </ul>
   *   </li>
   *   <li><b>Magnitude of change</b>:
   *     <ul>
   *       <li>Use gradients for colors: brighter means more change.</li>
   *     </ul>
   *   </li>
   * </ul>
   *
   ...
   */
  public Command createStandardDisplayCommand(BooleanSupplier isAlgeMode, Supplier<Gamepiece> gamepiece) {
    return newCommand(
  ...
  /**
   * Create a command that runs a roller animation.  Rainbows will scroll toward
   * the center for intake, and away from the center for outtake.
   * 
  ...
  public Command createRollerAnimationCommand(boolean isIntake) {
    ...
    return newCommand(() -> applyPattern(scrollingRainbow, isIntake ? intakeBuffers : outtakeBuffers))
        .withTimeout(duration);
  }
}
